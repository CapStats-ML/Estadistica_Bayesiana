...
nuEst = 3; mu0 = 250; eta0 = 1; alpha0 = 1; alpha0 = 1; lambda0 = 1; beta0 = 1/250
...
for (i in 1:m){
  ...
  Den = 1/(r/tau + 0.004)
  #mu0/gamma0 = 6500
  mu = rnorm(n = 1, mean = (sum(theta)/tau + 6500) * Den, sd = sqrt(Den)) 
  
  # ===> Actualizar tau^2
  # nu0 * tau0^2 = 250
  tau = 1/rgamma(n = 1, shape = 1/2 * (eta0 + r), rate = 125 + 1/2 * sum((theta - mu)^2))
  ...
  lpnu = nus0 * r/2 * log( nus0 * sigma * 0.5) - r * log(gamma(nus0 * 0.05)) + nus0 * 0.5 * sum(log(sigmaj)) -
    nus0 * (lambda0  + sigma/2 * sum(1/sigmaj))
  nu = sample(x = nus0, size = 1, prob = exp(lpnu - max(lpnu)))

  # ===> Actualizando sigma^2 
  # alpha0/2 = 0.5
  sigma = rgamma(n = 1, shape = 0.5 + r * nu * 1/2, rate = 0.02 + nu * sum(sigmaj) * 1/2)
